...
export default function TrailContent({ object, name, litigation_obj }) {
  ...
  // useEffect(() => {
  //   if (object?.event_id?.event_name) {
  //     const eventName = object.event_id.event_name;
  //     if (eventName.toLowerCase().endsWith(" date")) {
  //       eventName = eventName.slice(0, -5);
  //     }
  //     setTitle1(`Date of ${eventName}`);
  //   } else if (object?.parent_act?.event_id?.event_name) {
  //     setTitle1(`RE: ${object?.parent_act?.event_id?.event_name}`);
  //   } else {
  //     setTitle1("Label");
  //   }
  // }, [object]);
