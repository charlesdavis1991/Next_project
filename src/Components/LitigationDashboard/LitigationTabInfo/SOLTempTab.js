...
export default function SOLTempTab({ object, name, litigation_obj }) {
  ...
  }, [isLitigationDataUpdate, clientId, caseId]);
  ...
  if (object?.trigger_type === "DependantDate" && object?.dependant_date) {
    litigationActData.push({
      label: getLabelByDateType(
        object?.dependant_date_type?.dependant_date_name
      ),
      value: formatDateForPanelDisplay(object?.dependant_date),
    });
  } else if (object?.trigger_type === "DependantDate" && object?.rejected_on) {
    litigationActData.push({
      label: `Expired on`,
      value: formatDateForPanelDisplay(object?.rejected_on),
    });
  }
  // Apply to "Accept/Reject" trigger type
    ...
    litigationActData.push({
      ...
      value: formatDateForPanelDisplay(object?.accepted_on),
    });
  } else if (
    object?.trigger_type === "Accept/Reject" &&
    new Date(object?.rejected_on) > new Date(object?.end_date)
  ) {
    litigationActData.push({
      label: `Expired on`,
      value: formatDateForPanelDisplay(object?.rejected_on),
  ...
  const litigationActButtonConfig = [
    {
      ...
      onClick: () =>
        console.log("No event listener applied on this gen-doc btn"),
    },
  ];
  ...
  return (
    <div
      ...
      key={object?.id}
    >
      <TitleBar object={object} name={name} type="sol" />
            ...
            data={
              object?.calculated_dates?.length > 0
                ...
                : object?.event_id?.calculated_dates_id?.length > 0
                  ? object.event_id.calculated_dates_id.map(
                      (calDate, index) => {
                        return {
