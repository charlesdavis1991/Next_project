...
import {
  deleteJurisdiction,
  updateJurisdiction,
} from "../../Redux/jurisdiction-table/jurisdictionSlice";
...
function JurisdictionTableModal({
  jurisdictionPopUp,
  handleClose,
  jurisdictionData,
}) {
  ...
  useEffect(() => {
    if (jurisdictionPopUp && jurisdictionData) {
      setForm({
        id: jurisdictionData.id || "",
        name: jurisdictionData.name || "",
        states: jurisdictionData.states || [],
        counties: jurisdictionData.counties || [],
        circuits: jurisdictionData.circuits || [],
        districts: jurisdictionData.districts || [],
        divisions: jurisdictionData.divisions || [],
        jurisdiction_type: jurisdictionData.jurisdiction_type || {},
      });
    }
  }, [jurisdictionPopUp, jurisdictionData]);
  ...
  const getCircuits = async () => {
    try {
      const response = await axios.get(`${origin}/api/get/all/circuits/`, {
        headers: {
          Authorization: tokenBearer,
        },
      });
  ...
  const handleDeleteButton = async () => {
    try {
      const response = await axios.delete(
        `${origin}/api/add/jurisdiction/directory/${clientId}/${currentCaseId}/?id=${jurisdictionData.id}`,
        {
          headers: {
            Authorization: tokenBearer,
          },
      ...
      handleClose();
      dispatch(deleteJurisdiction(jurisdictionData.id));
      setActiveTab("jurisdiction");
    } catch (error) {
...
export default JurisdictionTableModal;
