...
import { addLitigation } from "../../Redux/litigation-event/litigationEventSlice";
...
function LitigationEventModal({ litigationEventPopUp, handleClose }) {
  ...
  const { litigations } = useSelector((state) => state.litigationEvent);
  ...
  useEffect(() => {
    if (litigationEventPopUp && selectedState) {
      getCountyData();
      getCalculatedDates();
    }
  }, [selectedState, litigationEventPopUp]);
  ...
  const handleLitigationEventSubmit = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        handleClose();
        dispatch(addLitigation(response.data.data));
  ...
  return (
    <Modal
      show={litigationEventPopUp}
      onHide={handleClose}
      dialogClassName="modal-dialog-centered max-width-1900px"
    >
      <Modal.Header className="bg-primary p-2">
        <h4 className="modal-title text-white mx-auto">Add Litigation Event</h4>
      </Modal.Header>
      <Modal.Body>
