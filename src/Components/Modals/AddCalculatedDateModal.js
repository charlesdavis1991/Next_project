...
function AddCalculatedDatesModal({ calculatedDatePopUp, handleClose }) {
  ...
  useEffect(() => {
    if(state?.StateAbr && calculatedDatePopUp){
      getCountiesData();
      getDependantDatesData();
    }
  }, [state, calculatedDatePopUp]);
  ...
  const handleStateChange = (e) => {
    ...
    setForm((prevForm) => {
      const selectedState = statesData?.find((state) => state.id === parseInt(value)) || null;
      if (selectedState) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.status === 201) {
        dispatch(addCalculatedDate(response.data.data));
        handleClose();
  ...
  return (
    ...
    >
      <Modal.Header className="bg-primary text-center p-2">
        <h4 className="modal-title text-white mx-auto">Add Calculated Date</h4>
      </Modal.Header>
      <Modal.Body>
        <Tab.Container
          activeKey={activeTab}
...
export default AddCalculatedDatesModal;
