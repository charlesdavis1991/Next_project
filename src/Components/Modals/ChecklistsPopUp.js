...
import { deleteChecklist, updateChecklist } from "../../Redux/checklists-table/checklistsSlice";

function ChecklistsPopUp({ editChecklist, handleClose, checklistData }) {
  ...
  useEffect(() => {
    if (editChecklist && checklistData) {
      console.log(checklistData);
      setForm({
        id: checklistData?.id || null,
        name: checklistData?.name || "",
        page: checklistData?.page?.id || "",
        order: checklistData?.order || "",
        is_extra_credit: checklistData?.is_extra_credit || false,
        blocking: checklistData?.blocking || false,
      });
    }
  }, [editChecklist, checklistData]);
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(
        `${origin}/api/checklists/directory/?id=${checklistData.id}`,
        {
          headers: {
            Authorization: tokenBearer,
          },
      ...
      if (response.status === 204) {
        handleClose();
        dispatch(deleteChecklist(checklistData.id));
...
export default ChecklistsPopUp;
