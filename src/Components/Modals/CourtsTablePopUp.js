...
import {
  deleteCourt,
  updateCourt,
} from "../../Redux/courts-table/courtsSlice";
...
function CourtsTablePopUp({ courtPopUp, handleClose, courtData }) {
  ...
  useEffect(() => {
    if (courtData) {
      setForm({
        id: courtData?.id || null,
        court_name: courtData?.court_name || "",
        court_county: courtData?.court_county || "",
        ...
        phone_number: courtData?.court_contact?.phone_number || "",
        phone_ext: courtData?.court_contact?.phone_ext || "",
        ...
        jurisdiction_type: courtData?.jurisdictiontype || {},
        court_title_1: courtData?.court_title_1 || "",
        court_title_2: courtData?.court_title_2 || "",
        counties_served:
          courtData?.counties_served?.map((county) => county.id) || [],
      });
      setForm_State(courtData?.court_contact?.state);
    }
  }, [courtData]);
  ...
  return (
    ...
    >
      <Modal.Header className="text-center bg-primary p-2">
        <h4 className="modal-title text-white mx-auto">Edit Court</h4>
      </Modal.Header>
      <Modal.Body>
        <Tab.Container
          activeKey={activeTab}
