...
import { addAct } from "../../Redux/act-table/actsSlice";
...
function ActModal({ addActs, handleClose }) {
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post(`${origin}/api/acts/directory/`, form, {
        headers: {
          Authorization: tokenBearer,
        },
      });
      if (response.status === 201) {
        dispatch(addAct(response.data.data));
        handleClose();
      }
    } catch (err) {
  ...
  return (
    ...
    >
      <Modal.Header className="bg-primary text-center p-2">
        <h4 className="modal-title mx-auto text-white">Add Acts</h4>
      </Modal.Header>
      <Modal.Body>
          ...
          <Nav variant="tabs" className="justify-content-center mb-2">
            ...
            <Nav.Item>
              <Nav.Link eventKey="trigger-missing">
                Trigger Missing Status
              </Nav.Link>
            </Nav.Item>
            <Nav.Item>
              <Nav.Link eventKey="block-missing">Block Missing Status</Nav.Link>
            </Nav.Item>
            <Nav.Item>
