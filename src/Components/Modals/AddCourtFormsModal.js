...
import { addCourtForm } from "../../Redux/courtForms-table/courtFormsSlice";
...
function AddCourtFormsModal({ courtFormPopUp, handleClose }) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.status === 201) {
        handleClose();
        dispatch(addCourtForm(response.data.data));
        setActiveTab("form");
  ...
  return (
    ...
    >
      <Modal.Header className="bg-primary text-center p-2">
        <h4 className="modal-title text-white mx-auto">Add Court Form</h4>
      </Modal.Header>
      <Modal.Body>
        <Tab.Container
          activeKey={activeTab}
                ...
                <div className="col-10">
                  ...
                  >
                    {jurisdictionData ? (
                      ...
                      <div>
                        No Jurisdictions available for the selected states
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  >
                    <p className="text-center">
                      {form.form && typeof form.form !== "string"
                        ? `Uploaded: ${form.form?.name}` // Show the file name if it's a file object
...
export default AddCourtFormsModal;
