...
function getStyles() {
  return {
    height: window.innerWidth > 2300 ? "21px" : "40px",
...
const LeastAccess = ({ chats }) => {
  ...
  const [leastCases, setLeastCases] = useState([]);
  ...
  const fetchLeastCases = async () => {
    try {
      const response = await api.get(
        `${origin}/api/homepage/least-accessed-cases/`
      );

      if (response.status === 200) {
        var data = response.data;
        console.log("fetchLeastCases", data);
        setLeastCases(data);
      }
    } catch (error) {
  ...
  const calculateRowHeight = () => (window.innerWidth > 2300 ? 21 : 40);
  ...
  return (
    <div className="column order-2 order-xxl-1 d-flex flex-column m-b-5">
      ...
      <div
        className="table-responsive table--no-card position-relative border-0 has-tint-rows has-tint-h-35 flex-g-1"
        style={divStyle}
      >
        {leastCases.length > 0 ? (
          <table className="table table-earning position-relative z-index-1 position-relative">
            ...
            <tbody id="group_chat_body">
                  ...
                  if (
                    ...
                    return (
                      ...
                      >
                        ...
                        <td
                          className="text-black d-flex pb-0 pt-0"
                          style={style}
                        >
                        ...
                        <td
                          ...
                          id={`${className}`}
                        >
                          {caseData?.last_accessed
                            ? standardFormatDate(caseData?.last_accessed)
                            : ""}
                        </td>
                      </tr>
                    );
              ...
              {leastCases.length < calculateRowsToDisplay() &&
                Array.from(
                  { length: calculateRowsToDisplay() - leastCases.length },
                  (_, index) => (
                    <tr key={`fake-${index}`}>
                      <td
                        className=""
